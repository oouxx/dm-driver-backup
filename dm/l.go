...
func newClobFromDB(value []byte, conn *DmConnection, column *column, fetchAll bool) *DmClob {
	...
	clob.inRow = Dm_build_1248.Dm_build_1341(value, NBLOB_HEAD_IN_ROW_FLAG) == LOB_IN_ROW
	clob.blobId = Dm_build_1248.Dm_build_1355(value, NBLOB_HEAD_BLOBID)
	if !clob.inRow {
		clob.groupId = Dm_build_1248.Dm_build_1345(value, NBLOB_HEAD_OUTROW_GROUPID)
		clob.fileId = Dm_build_1248.Dm_build_1345(value, NBLOB_HEAD_OUTROW_FILEID)
		clob.pageNo = Dm_build_1248.Dm_build_1350(value, NBLOB_HEAD_OUTROW_PAGENO)
	}
	if conn.NewLobFlag {
		clob.tabId = Dm_build_1248.Dm_build_1350(value, NBLOB_EX_HEAD_TABLE_ID)
		clob.colId = Dm_build_1248.Dm_build_1345(value, NBLOB_EX_HEAD_COL_ID)
		clob.rowId = Dm_build_1248.Dm_build_1355(value, NBLOB_EX_HEAD_ROW_ID)
		clob.exGroupId = Dm_build_1248.Dm_build_1345(value, NBLOB_EX_HEAD_FPA_GRPID)
		clob.exFileId = Dm_build_1248.Dm_build_1345(value, NBLOB_EX_HEAD_FPA_FILEID)
		clob.exPageNo = Dm_build_1248.Dm_build_1350(value, NBLOB_EX_HEAD_FPA_PAGENO)
	}
	...
	if clob.inRow {
		if conn.NewLobFlag {
			clob.data = []rune(Dm_build_1248.Dm_build_1405(value, NBLOB_EX_HEAD_SIZE, int(clob.getLengthFromHead(value)), clob.serverEncoding, conn))
		} else {
			clob.data = []rune(Dm_build_1248.Dm_build_1405(value, NBLOB_INROW_HEAD_SIZE, int(clob.getLengthFromHead(value)), clob.serverEncoding, conn))
		}
...
func (clob *DmClob) WriteString(pos int, s string) (n int, err error) {
	...
	if clob.local || clob.fetchAll {
		...
		var writeLen, err = clob.connection.Access.dm_build_530(clob, pos, s, clob.serverEncoding)
		if err != nil {
			return -1, err
		}
...
func (clob *DmClob) Truncate(length int64) error {
	...
	if clob.local || clob.fetchAll {
		...
		clob.length, err = clob.connection.Access.dm_build_560(&clob.lob, int(length))
		if err != nil {
			return err
		}
		if clob.groupId == -1 {
...
func (clob *DmClob) getSubString(pos int64, len int32) (string, error) {
	...
	if clob.local || clob.inRow || clob.fetchAll {
		...
		return clob.connection.Access.dm_build_519(clob, int32(pos), len)
	}
}
