...
func (conn *DmConnection) setTrxFinish(status int32) {
	switch status & Dm_build_733 {
	case Dm_build_730, Dm_build_731, Dm_build_732:
		conn.trxFinish = true
	default:
...
func (dc *DmConnection) executeInner(query string, execType int16) (interface{}, error) {
	...
	if execType == Dm_build_697 && dc.dmConnector.enRsCache {
	...
	if optParamList != nil && len(optParamList) > 0 {
		info, err = dc.Access.Dm_build_415(stmt, optParamList)
		if err != nil {
			stmt.nativeSql = query
			info, err = dc.Access.Dm_build_421(stmt, execType)
		}
	} else {
		info, err = dc.Access.Dm_build_421(stmt, execType)
	}
...
func (dc *DmConnection) reconnect() error {
	...
	for _, stmt := range dc.stmtMap {
		err = dc.Access.Dm_build_393(stmt)
		if err != nil {
			return err
		}
...
func (dc *DmConnection) exec(query string, args []driver.Value) (*DmResult, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_698)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) query(query string, args []driver.Value) (*DmRows, error) {
	...
	if args != nil && len(args) > 0 {
		...
		r1, err := dc.executeInner(query, Dm_build_697)
		if err != nil {
			return nil, err
		}
...
func (dc *DmConnection) driverQuery(query string) (*DmStatement, *DmRows, error) {
	...
	stmt.innerUsed = true
	stmt.innerExec = true
	info, err := dc.Access.Dm_build_421(stmt, Dm_build_697)
	if err != nil {
		return nil, nil, err
	}
	dc.lastExecInfo = info
