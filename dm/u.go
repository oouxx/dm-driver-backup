...
func (rpv rsPoolValue) refreshed(conn *DmConnection) (bool, error) {
	...
	tss, err := conn.Access.Dm_build_486(interface{}(rpv.m_TbIds).([]uint32))
	if err != nil {
		return false, err
	}
	rpv.m_lastChkTime = time.Now().Nanosecond()
...
func (st *DmStatement) prepare() error {
	...
	st.execInfo, err = st.dmConn.Access.Dm_build_411(st, Dm_build_696)
	if err != nil {
		return err
	}
	return nil
...
func (stmt *DmStatement) exec(args []driver.Value) (*DmResult, error) {
	...
	if stmt.isBatch && len(args) > 0 {
		...
		err = stmt.executeInner(args, Dm_build_698)
	}
	if err != nil {
		return nil, err
	}
...
func (stmt *DmStatement) query(args []driver.Value) (*DmRows, error) {
	var err error
	stmt.inUse = true
	err = stmt.executeInner(args, Dm_build_697)
	if err != nil {
		return nil, err
	}
...
func NewDmStmt(conn *DmConnection, sql string) (*DmStatement, error) {
	...
	if s == nil {
		...
		if conn.stmtPool != nil && len(conn.stmtPool) > 0 {
			...
			err := conn.Access.Dm_build_393(s)
			if err != nil {
				return nil, err
			}
		}
...
func (stmt *DmStatement) free() error {
	...
	err := stmt.dmConn.Access.Dm_build_398(int32(stmt.id))
	if err != nil {
		return err
	}
	delete(stmt.dmConn.stmtMap, stmt.id)
	...
	if bindParam.ioType == IO_TYPE_OUT || bindParam.ioType == IO_TYPE_UNKNOWN {
		...
		if bindParam.colType == CURSOR {
			...
			if bindParam.cursorStmt == nil {
				...
				err = bindParam.cursorStmt.dmConn.Access.Dm_build_393(bindParam.cursorStmt)
			}
		}
	}
	...
	nextSwitch:
		if stmt.serverParams[i].colType == CURSOR {
			...
			if stmt.bindParams[i].cursorStmt == nil {
				...
				err = stmt.bindParams[i].cursorStmt.dmConn.Access.Dm_build_393(stmt.bindParams[i].cursorStmt)
			}
			stmt.bindParams[i].ioType = IO_TYPE_INOUT
			continue
		}
...
func (stmt *DmStatement) executeInner(args []driver.Value, executeType int16) (err error) {
	...
	stmt.execInfo, err = stmt.dmConn.Access.Dm_build_443(stmt, bytes, false)
	if err != nil {
		return err
	}
		...
		for i, outParamData := range stmt.execInfo.outParamDatas {
			...
			if sc, ok := args[i].(sql.Scanner); ok {
				...
				case CURSOR:
					...
					if tmpExecInfo, err = stmt.dmConn.Access.Dm_build_453(stmt.bindParams[i].cursorStmt, 1); err != nil {
						return err
					}
				...
				case *driver.Rows:
					if stmt.bindParams[i].colType == CURSOR {
						var tmpExecInfo *execRetInfo
						tmpExecInfo, err = stmt.dmConn.Access.Dm_build_453(stmt.bindParams[i].cursorStmt, 1)
						if err != nil {
							return err
						}
...
func (stmt *DmStatement) executeBatch(args []driver.Value) (err error) {
	...
	if stmt.execInfo.retSqlType == Dm_build_711 || stmt.execInfo.retSqlType == Dm_build_716 {
	...
	if stmt.paramCount > 0 && args != nil && len(args) > 0 {
		...
		if len(args) == 1 || stmt.dmConn.dmConnector.batchType == 2 ||
			(stmt.dmConn.dmConnector.batchNotOnCall && stmt.execInfo.retSqlType == Dm_build_712) {
			...
			stmt.execInfo, err = stmt.dmConn.Access.Dm_build_432(stmt, bytes, stmt.preExec)
		}
	}
	return err
}
...
func (stmt *DmStatement) executeBatchByRow(args []driver.Value) (err error) {
	...
	for i := 0; i < count; i++ {
		tmpExecInfo, err := stmt.dmConn.Access.Dm_build_443(stmt, args[i].([]interface{}), stmt.preExec || i != 0)
		if err == nil {
